...
\begin{figure}[htp]
    \centering
    \includegraphics[width=\linewidth]{oauth-outline.drawio.pdf}
...
\begin{table}
    ...
    \begin{tabular}{cc}
        ...
        \Code{unauthorized_client}               & The client to authorise using this method.             \\
    ...
    \end{tabular}
    \caption{Error codes for authorisation code step OAuth2}
...
The response (Step 4) will be JSON format.

% TODO: OAuth 2.0

Step 5 and 6 will be repeated multiple times in the application, to request a new access token using the provided refresh token.
...
This is a GET request with no parameters to specify, and will return all stations in one go. Listing \ref{code:parameter-earthquake} shows a sample response.
