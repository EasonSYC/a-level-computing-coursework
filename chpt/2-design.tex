...
\begin{table}[!ht]
    ...
    \begin{tabular}{|c|p{0.65\linewidth}|}
        ...
        \Code{gd.eew}                  & Get list of past EEWs.                                                    \\
        \Code{parameter.earthquake}    & Get details of observation points for earthquake intensities.             \\
        \Code{parameter.tsunami}       & Get details of observation points for tsunamis.                           \\
...
\subparagraph{Contract-Related APIs}
...
\begin{itemize}
    \item \Code{id} which stands for the subscription ID (could be \Code{null} if the user is not subscribed);
    ...
    \item \Code{start} which stands for the start of the subscription;
...
\subparagraph{WebSocket-Related APIs}

There are three WebSocket-Related APIs, specifically, to start a socket, \Code{socket.start}, to list all sockets, \Code{socket.list} and to close a socket, \Code{socket.close}.

\subparagraph{Parameter-Related APIs}
